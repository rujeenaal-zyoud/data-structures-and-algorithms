...
const getNames = (arr) => {
  ...
  let nameArr = arr.map(elemet=>{
    //here after run elemt.name it show name in reverse so i do this steps that split then reverse then join 
    return elemet.name.split('').reverse().join(''); // need ask why
  });
  return nameArr;
};
